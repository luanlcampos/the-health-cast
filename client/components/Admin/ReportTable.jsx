...
export default function ReportTable({ user }) {
  ...
  const getReportList = async () => {
    try {
      ...
      const q = await query(
        reportsRef,
        where("reportedAccountOrg", "==", String(user.uid))
      );
      ...
      let i = 0;
      const reportList = reportsSnap.docs.map((report) => ({
        //array of reports for given admin (user.uid)
        ...report.data(),
        id: report.id,
      }));
      ...
      for (let i = 0; i < reportList.length; i++) {
        const acc = await getDoc(
          doc(db, "users", String(reportList[i].reportedAccountId))
        );
      ...
      console.log(
        `after all operations, displaying reports of org inside reportList.map:`
      );
  ...
  const removeHcpFromList = async (e, report) => {
    e.preventDefault();
    console.log(
      `removing report ${report.id} from ${report.reportedAccountId} -- hcps; orgId ${user.uid}`
    );
    openModal(
      "The selected report will be unavailable once deleted from your list"
    ).then(async (result) => {
      if (result.value) {
        try {
  ...
  const handlePermissionChange = async (e, report) => {
    ...
    const hcp = await getDoc(
      doc(db, "users", String(report.reportedAccountId))
    );
    hcp = hcp.data();
      ...
      <div className="container table-container overflow-auto">
        <div className="org-table">
          <div className="flex flex-row justify-between items-center">
            <h1 className="text-xl">Reports List</h1>
                    ...
                    <tr key={index} className="fade-enter">
                      ...
                      <td>
                        ...
                        <Select
                          options={options}
                          isDisabled={
                            enableEdit !== report.reportedAccountId
                              ? true
                              : false
                          }
                          value={{
                            ...
                            label:
                              report.permission[0].toUpperCase() +
                              report.permission.slice(1),
                          }}
                        ...
                        <div className="actions flex justify-around text-2xl">
                          <div className="hover:cursor-pointer">
                            <AiOutlineEdit
                              ...
                              onClick={(e) =>
                                toggleEdit(e, report.reportedAccountId)
                              }
                            />
                          </div>
                          <div className="hover:cursor-pointer">
