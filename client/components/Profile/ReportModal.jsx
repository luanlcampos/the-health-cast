  ...
  const handleSubmit = async (event) => {
    try {
      ...
      if (res.isConfirmed) {
        ...
        const reportingUserProfileData = await getDoc(
          doc(db, "users", String(user.uid))
        );
        reportingUserProfileData = reportingUserProfileData.data();
        console.log(
          "firstMonthlyReportDate",
          reportingUserProfileData.firstMonthlyReportDate,
          " | totalNumberReports",
        ...
        const canSubmitReport = async (date, numOfReportsSubmittedThisWeek) => {
          ...
          console.log(
            ...
            date,
            ` | `,
            numOfReportsSubmittedThisWeek
          );
          console.log(
            `canSubmitReport: `,
            currDate,
            ` | periodSinceLastReport: `,
            periodSinceLastReport,
            ` | 604800 secs in 1 week `
          );
          if (numOfReportsSubmittedThisWeek < 5) {
            return true;
          } else if (periodSinceLastReport > 7 * 24 * 60 * 60) {
            //604800 secs in 1 week
            return true;
          }
          return false;
        ...
        const subRep = await canSubmitReport(
          reportingUserProfileData.firstMonthlyReportDate,
          reportingUserProfileData.totalNumberReports
        );
        ...
        if (subRep) {
          ...
          console.log(
            "createReport:",
            " cannot submit report due to report submission limit"
          );
        }
