...
const createReceiverPeerConnection = (socketID, socket, roomID) => {
  ...
  pc.ontrack = (e) => {
    if (users[roomID]) {
      console.log("Room Exists....Checking if user already in the room");
    ...
    console.log(
      "Sending Message to the client that the user has entered the room, Creator first and then blah..."
    );
...
io.sockets.on("connection", (socket) => {
  socket.on("joinRoom", (data) => {
    try {
      ...
      console.log(`Current User: ${data.userFirstName} ${data.userLastName}`);
      console.log(`Current User ID: ${data.uuid}`);
    } catch (error) {
      console.log(error);
    }
  });
